...
echo "============================================"
"$PYTHON_BIN" code/pipeline/01_load_data.py --n 3000 --seed 42 --out data/01_sampled_source/klue_nli_validation_sample_3000.csv
...
for gen in clova gemini openai sonnet; do
    ...
    "$PYTHON_BIN" code/pipeline/02_generate_llm_attacks.py \
        --generator "$gen" \
...
for method in bert_attack backtranslation eda; do
    ...
    "$PYTHON_BIN" code/pipeline/03_generate_baselines.py \
        --method "$method" \
        --input data/01_sampled_source/klue_nli_validation_sample_3000.csv \
...
for gen in clova gemini openai sonnet; do
    gen_file="${MODEL_FILE_KEY[$gen]}"
    "$PYTHON_BIN" code/pipeline/04_validate_attacks.py \
...
echo "============================================"
"$PYTHON_BIN" code/pipeline/05_build_shared_valid.py
...
for tgt in clova gemini openai sonnet; do
    ...
    "$PYTHON_BIN" code/pipeline/06_evaluate_attacks.py \
        --cache_originals \
        --target "$tgt" \
...
echo "============================================"
"$PYTHON_BIN" code/analysis/02_verify_4x4_integrity.py --expected_rows 2209
"$PYTHON_BIN" code/analysis/03_generate_4x4_analysis.py
"$PYTHON_BIN" code/analysis/04_statistical_analysis_summary.py

echo ""
echo "============================================"
echo "Step 9: Back Translation Comparison"
echo "============================================"
"$PYTHON_BIN" code/analysis/01_prepare_bt_shared.py
...
echo "============================================"
"$PYTHON_BIN" code/analysis/05_calculate_quality_metrics.py
"$PYTHON_BIN" code/analysis/06_generate_quality_figure.py
...
if [[ -f "$human_majority_results" ]]; then
    "$PYTHON_BIN" code/analysis/12_compute_human_adjusted_asr.py
    "$PYTHON_BIN" code/analysis/13_write_bt_llm_comparison_table.py
else
